                ...
                <div class="col-md-3">
                    <aside class="sidebar">
                        <ul class="nav nav-list mb-xlg">
                            ...
                            <li><a href="?pag=concurso_motorista">Motorista</a></li>
                            <li class="active"><a href="?pag=concurso_servente">Servente</a></li>
                            <li><a href="?pag=concurso_jardineiro">Jardineiro</a></li>
                        </ul>    
                    </aside>
                </div>      
                <div class="col-md-9">
                    <h2>Concurso | Servente</h2>
                    <hr class="tall">
                    <div class="row">
                        <div class="row">
                            <?php
                            $connected = mysqli_connect('192.168.1.14:3306', 'matabala', 'matabala', 'db_site_mees');
                            $noticias = mysqli_query($connected, "select ID, TITULO, TEXTO,IMAGEN, ESTADO,CONCURSO,DATA_REG from view_ver_concurso  WHERE CONCURSO='servente' ");
                            while ($linha = mysqli_fetch_assoc($noticias)) {
                                $ID = $linha['ID'];
                                $TITULO = $linha['TITULO'];
                                $strcut = substr($linha['TEXTO'], 0, 300);
                                $texto1 = substr($strcut, 0, strrpos($strcut, ' ')) . '....';
                                $data = date("D M j G:i:s T Y", strtotime($linha["DATA_REG"]));
